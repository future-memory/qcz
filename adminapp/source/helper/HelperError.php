...
class HelperError
{
	...
	{
		if(!empty($message)) {
			$message = HelperLang::lang('error', $message);
		} else {
			$message = HelperLang::lang('error', 'error_unknow');
		}
	...
	{
		$skipfunc[] = 'HelperError->debug_backtrace';
		$skipfunc[] = 'HelperError->db_error';
		$skipfunc[] = 'HelperError->template_error';
		$skipfunc[] = 'HelperError->system_error';
		...
		foreach ($debug_backtrace as $k => $error) {
			...
			$error['line'] = sprintf('%04d', isset($error['line']) ? $error['line'] : 0);
	...
	public static function db_error($message, $sql) 
	{
		...
		$title = HelperLang::lang('error', 'db_'.$message);
		$title_msg = HelperLang::lang('error', 'db_error_message');
		$title_sql = HelperLang::lang('error', 'db_query_sql');
		$title_backtrace = HelperLang::lang('error', 'backtrace');
		$title_help = HelperLang::lang('error', 'db_help_link');
	...
	public static function exception_error($exception) 
	{
		...
		foreach ($trace as $error) {
			...
			$phpmsg[] = array(
			    'file' => str_replace(array(BASE_ROOT, '\\'), array('', '/'), $error['file']),
			    'line' => $error['line'],
	...
	public static function write_error_log($message) 
	{
		$message = self::clear($message);
		$time    = time();
		$file    =  BASE_ROOT.'/data/log/'.date("Ymd").'_error_log.php';
		$hash    = md5($message);
		...
		$message = "<?PHP exit;?>\t".date("Y-m-d H:i:s")."\t$message\t$hash\t$user $uri\r\n";
		...
		if(($fp = @fopen($file, 'rb'))!=false) {
			...
			if(($data = fread($fp, $lastlen))!=false) {
				$array = explode("\r\n", $data);
	...
	public static function show_error($type, $errormsg, $phpmsg = '', $typemsg = '') 
	{
		...
		if(!defined('DEBUG_MOD') || DEBUG_MOD!==TRUE){	// 禁止输出到页面上
			header("Location: ".PAGE_SYS_ERROR);
			die ;
		}
...
EOT;
		...
		$endmsg = HelperLang::lang('error', 'error_end_message', array('host'=>$host));
		echo <<<EOT
